...
| :--- | :--- | :--- | :---: | :---: |
| **1. CLI Menu System** | Menu tương tác dùng vòng lặp vô hạn, xử lý lỗi nhập. | Sử dụng vòng lặp `while True` trong hàm `main()` và khối `else` để báo lỗi khi người dùng nhập sai lựa chọn. | ✅ | 1.0 |
| **2. Data Input & Validation** | Thêm bản ghi mới và xác thực kiểu dữ liệu. | Hàm `kiem_tra_sdt` dùng Regex để chặn định dạng sai; hàm `them_lien_he` kiểm tra tên trống và SĐT trùng lặp. | ✅ | 1.0 |
| **3. Data Display** | In bản ghi rõ ràng, căn lề chuẩn theo bảng. | Hàm `hien_thi_danh_ba` sử dụng `f-string` với định dạng căn lề trái (ví dụ: `:<25`) để các cột luôn thẳng hàng. | ✅ | 1.0 |
| **4. Basic Search** | Tìm kiếm theo ID hoặc Tên chính xác. | Hàm `xoa_lien_he` và logic trong quản lý sử dụng so sánh trực tiếp để tìm chính xác liên hệ theo mã định danh. | ✅ | 1.0 |
| **5. Sorting Mechanism** | Sắp xếp theo ít nhất một trường dữ liệu. | Hàm `sap_xep_danh_ba` kết hợp với `vietnamese_sort_key` để sắp xếp tên theo chuẩn bảng chữ cái Tiếng Việt (A-Z). | ✅ | 1.0 |
| **6. Basic Calculation** | Tính toán thống kê cơ bản (tổng, đếm). | ]Hàm `thong_ke` sử dụng hàm `len(contacts)` để tính tổng số lượng bản ghi hiện có trong hệ thống. | ✅ | 1.0 |
| **7. TXT File Handling** | Lưu và tải dữ liệu từ tệp .txt bền vững. | Triển khai qua hai hàm `tai_du_lieu` (đọc tệp khi khởi động) và `luu_du_lieu` (ghi lại thay đổi sau mỗi thao tác). | ✅ | 1.0 |
| **8. [Advanced] Complex Logic** | Tìm kiếm chuỗi con hoặc thống kê theo nhóm. | Hàm `tim_kiem_va_loc` hỗ trợ tìm kiếm khớp một phần (substring) và `thong_ke` phân loại liên hệ theo tên miền email. | ✅ | 1.0 |
| **9. [Advanced] JSON/DBMS** | Xuất/Nhập dữ liệu dùng tệp cấu trúc .json. | Hàm `xuat_json` sử dụng thư viện `json` của Python để chuyển đổi danh sách dictionary thành tệp JSON có định dạng. | ✅ | 1.0 |
| **10. Git & Modular Code** | Dùng GitHub, có README, code chia theo hàm. | Code được thiết kế theo kiểu Top-Down, mỗi hàm đảm nhận một nhiệm vụ duy nhất (Single Responsibility). | ✅ | 1.0 |
| **TỔNG CỘNG** | | | | **10.0** |
